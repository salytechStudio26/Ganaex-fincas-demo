/* ============================================================
   BOTÓN BUSCAR — LOTES DE ANIMALES (VERSIÓN ADAPTADA)
   ============================================================ */
...
document.getElementById("btnBuscarAnimales").onclick = async () => {
    ...
    const modo = prompt(
        ...
        "1 - Búsqueda individual (ID o Nº lote)\n" +
...
async function buscarIndividual() {
    ...
    const dato = prompt("Introduce el ID o Nº lote:");
    ...
    const resultado = todos.filter(reg =>
        String(reg.id) === dato.trim() ||
        String(reg.lote).toLowerCase() === dato.trim().toLowerCase()
    );

    if (resultado.length === 0) {
        alert("No se encontró ningún lote.");
        return;
    }
...
async function mostrarPanelBusquedaAvanzada() {
    ...
    panel.innerHTML = `
        ...
        ${crearSelect("Nº lote", "lote", valoresUnicos.lote)}
        ${crearSelect("Cantidad", "cantidad", valoresUnicos.cantidad)}
...
function obtenerValoresUnicos(registros) {
    ...
    registros.forEach(r => {
        ...
        if (r.lote) lote.add(r.lote);
        if (r.cantidad) cantidad.add(r.cantidad);
        ...
        Object.keys(r).forEach(col => {
            if (!["id","lote","cantidad","sexo","zona","observaciones"].includes(col)) {
...
function pintarResultadosBusqueda(lista) {
    ...
    lista.forEach(reg => {
        ...
        const extras = Object.keys(reg).filter(
            c => !["id","lote","cantidad","sexo","zona","observaciones"].includes(c)
        );
