/* ============================================================
   BOTÓN BUSCAR — CRÍAS INDIVIDUALES (VERSIÓN FINAL)
   ============================================================ */
...
const btn = document.getElementById("btnBuscarAnimales");
if (!btn) {
...
async function buscarIndividual() {
    ...
    const resultado = todos.filter(cria =>
        String(cria.id) === dato.trim() ||
        String(cria.identificacion).toLowerCase() === dato.trim().toLowerCase()
    );

    if (resultado.length === 0) {
        alert("No se encontró ninguna cría.");
        return;
    }
...
async function mostrarPanelBusquedaAvanzada() {
    ...
    panel.innerHTML = `
        ...
        ${crearSelect("Tipo entrada", "tipoEntrada", valoresUnicos.tipoEntrada)}
        ${crearSelect("Crotal madre", "crotalMadre", valoresUnicos.crotalMadre)}
...
function obtenerValoresUnicos(registros) {
    ...
    const tipoEntrada = new Set();
    const crotalMadre = new Set();
    const sexo = new Set();
    const raza = new Set();
    const zona = new Set();
    ...
    registros.forEach(r => {
        ...
        if (r.tipoEntrada) tipoEntrada.add(r.tipoEntrada);
        if (r.crotalMadre) crotalMadre.add(r.crotalMadre);
        ...
        Object.keys(r).forEach(col => {
            if (![
                "id","identificacion","fechaNacimiento","tipoEntrada",
                "crotalMadre","raza","sexo","zona","observaciones"
            ].includes(col)) {
...
function pintarResultadosBusqueda(lista) {
    ...
    lista.forEach(reg => {
        ...
        fila.innerHTML = `
            ...
            <td>${reg.tipoEntrada ?? ""}</td>
            <td>${reg.crotalMadre ?? ""}</td>
            <td>${reg.raza ?? ""}</td>
            <td>${reg.sexo ?? ""}</td>
            <td>${reg.zona ?? ""}</td>
